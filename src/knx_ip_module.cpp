...
bool KNXIPModule::sendKNXMessage(int groupAddress, const uint8_t* data, size_t dataLength) {
    ...
    if (connectionType == KNX_CONNECTION_UNICAST) {
        ...
        buffer[bufferLength++] = 0x04; // Service type (tunneling request)
        buffer[bufferLength++] = 0x20; // Reserved
        ...
        // Tunneling request
        buffer[bufferLength++] = 0x04; // Structure length
        buffer[bufferLength++] = 0x00; // Channel ID
        buffer[bufferLength++] = 0x00; // Sequence counter
        buffer[bufferLength++] = 0x00; // Reserved

        // KNX cEMI data (starts here)
        buffer[bufferLength++] = 0x11; // Message code L_Data.req
        buffer[bufferLength++] = 0x00; // Additional info length (0)
        
        buffer[bufferLength++] = 0xBC; // Control field 1 - Standard frame, no repeat, Normal priority, no ACK
        buffer[bufferLength++] = 0xE0; // Control field 2 - Group address + hop count 6
        ...
        buffer[bufferLength++] = dataLength + 1; // TPDU length (data + 1 for TPCI/APCI)
        
        // Copy data - first byte includes TPCI/APCI
        ...
        buffer[bufferLength++] = 0x30; // Reserved
        ...
        // KNX cEMI data (starts here)
        buffer[bufferLength++] = 0x29; // Message code (L_Data.ind)
        buffer[bufferLength++] = 0x00; // Additional info length (0)
...
void KNXIPModule::processUdpData(AsyncUDPPacket packet) {
    ...
    // Check packet type
    if (packet.data()[0] == 0x06 && serviceType == 0x04) {
...
KNXTelegram KNXIPModule::parseTelegram(const uint8_t* data, size_t length) {
    ...
    int offset = 0;
    if (data[0] == 0x29 || data[0] == 0x11) {  // L_Data.ind or L_Data.req message code
    ...
    telegram.isGroupAddress = (ctrl2 & 0x80) != 0;
...
float KNXIPModule::decodeDPT9(const uint8_t* data, size_t length) {
    ...
    uint16_t dpt9Value = (data[2] << 8) | data[3];
